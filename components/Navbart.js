...
const Navbart = () => {
    ...
    const [setshowdrop, setSetshowdrop] = useState(false)
    ...
    return (
        <nav className='bg-gray-900 text-white flex justify-between px-4 h-16 items-center '>
            ...
            <div className='relative'>
                ...
                {session && <><button onClick={() => setSetshowdrop(!setshowdrop)} onBlur={() => {
            setTimeout(() => {
              setSetshowdrop(false)
            }, 100);
                    ...
                    <div id="dropdown" className={`z-10 bg-white divide-y ${setshowdrop ? "" : "hidden" } divide-gray-100 rounded-lg shadow-sm w-44 dark:bg-gray-700 absolute left-[150px]`}>
                ...
                {session && <button className="text-white bg-gradient-to-br from-purple-600 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2 cursor-pointer" onClick={() => { signOut() }} >Sign Out</button>
                }
